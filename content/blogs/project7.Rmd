...
- ""
date: 2020-09-07
description: Analysis of AirBnB property prices in Munich
draft: false
image: airbnb.jpg
keywords: ""
slug: Project7
title: "Analysis of AirBnB property prices in Munich"
---
...
Let us start by taking a quick look to our cleansed dataframe and all the variables that we have. 

``` {r  EDA glimpse}
glimpse(final_listings) #using glimpse function for raw view
```

Let us now quickly review our variables more in depth. 
Function skim allows to get a better view of the data: we can now have an idea of missing values, variable types and other useful information.

``` {r  EDA fskim}
skim(final_listings) #more detailed view using function skim
```

From this table, it appears that our dataset has 24 variables and  9,376 observations.The dataset is composed of 11 numerical variables, 5 factors ,3 characters, 4 logical and 1 data variable. 
...
``` {r EDA favstats}
  ...
  favstats(price~cleaning_fee, data=final_listings) 
...
bestmodel <- lm(log(price_4_nights) ~ prop_type_simplified  + review_scores_rating + bedrooms + accommodates + room_type + cleaning_fee + instant_bookable  + cancellation_policy + neighbourhood_cleansed + availability_365 + availability_90 + last_review, data = learn)
summary(bestmodel)
...
The RMSE comes out to be 401.4919.
...
predicted_prices <- predict(questioned_model, newdata = questioned_listings, interval = "confidence") %>% 
  exp()
